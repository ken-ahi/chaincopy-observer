...
Discovery候補を無料データ範囲で追加するときは、`pnpm hl:promote-free-candidates`をdry-runする。対象は既存正式filterが`ELIGIBLE`、enrichmentが`SUCCEEDED`、historyが`COMPLETE`、non-truncated、DQ score 100、候補OPEN DQなし、Selection評価期間とDiscovery recent activityを満たすものに限る。さらにInfo APIで候補の`availableFrom`を始点にbounded fill取得を行い、coverageが証明され、最初のfillを一意に特定でき、その`startPosition`がcanonical zeroであることを必須とする。`perpAllTime` portfolio historyの最初のNAV日もfill開始日以前でなければならない。`tid`はidentityであり、境界のcausal ordering根拠にはしない。dry-run manifestには最初のfillのtimestamp / `tid` / `startPosition`と最初のNAV timestampを含め、実行はcount/hash一致を必須とする。候補走査は最低評価期間を満たす新しい開始境界から`--scan-limit`上限まで行い、promotion後の履歴取得は通常の`walletBackfill`を使う。backfillで10,000 Fill上限や他の不完全性が判明したwalletは`REVIEW`のまま扱う。
