...
Discovery候補を無料データ範囲で追加するときは、`pnpm hl:promote-free-candidates`をdry-runする。対象は既存正式filterが`ELIGIBLE`、enrichmentが`SUCCEEDED`、historyが`COMPLETE`、non-truncated、DQ score 100、候補OPEN DQなし、Selection評価期間とDiscovery recent activityを満たすものに限る。さらにInfo APIで候補の`availableFrom`を始点にbounded fill取得を行い、coverageが証明され、最初のfillを一意に特定でき、その`startPosition`がcanonical zeroであることを必須とする。`tid`はidentityであり、境界のcausal ordering根拠にはしない。dry-run manifestには最初のfillのtimestamp / `tid` / `startPosition`を含め、実行はcount/hash一致を必須とする。候補走査は`--scan-limit`で上限を設け、promotion後の履歴取得は通常の`walletBackfill`を使う。backfillで10,000 Fill上限や他の不完全性が判明したwalletは`REVIEW`のまま扱う。
